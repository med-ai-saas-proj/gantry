...
def get_effective_roles(user_roles: list[str]) -> set[str]:
    ...
    Example:
        >>> get_effective_roles(["member.admin"])
            ...
            if role_enum in ROLE_HIERARCHY:
                ...
                for child_role in child_roles:
                    if child_role in ROLE_HIERARCHY:
                        ...
                        effective_roles.update(r.value for r in grandchild_roles)
        except ValueError:
...
def has_role(user_roles: list[str] | None, required_role: ManagementRole) -> bool:
    ...
    Example:
        >>> has_role(["member.admin"], ManagementRole.MEMBER_ADD)
        True
        >>> has_role(["member.view"], ManagementRole.MEMBER_ADD)
        False
    """
    if not user_roles:
        return False
...
def has_any_role(
    ...
    required_roles: list[ManagementRole]
) -> bool:
