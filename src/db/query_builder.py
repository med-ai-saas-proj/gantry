...
SqlColumnInterface = (
    str | SqlText | list[str] | list[SqlText] | tuple[str] | tuple[SqlText]
)
...
class BaseSqlQueryBuilder:
    def __init__(self, table: str, schema: str | None):
        ...
        if self.schema is not None:
            ...
            self.full_table_name = (
                f"{literal_object(self.schema)}.{literal_object(self.table)}"
            )
        else:
    ...
    def values(
        ...
        text_clauses: Optional[dict[str, SqlText]] = None,
        execute_batch=False,
    ):
        if len(records) == 0:
            return SqlQuery(sql="")
    ...
    def where(cls, conditions: SqlConditionInterface, alias=None):
        ...
        for sub_condition in conditions["conditions"]:
            if "conditions" in sub_condition:
                ...
                sub_query = SqlQuery(
                    sql=sub_sql, params=sub_params, columns=None
                )
