...
def create_access_token(data: JwtPayload, secret_key: str, algorithm: str, expires_delta: int | None = None) -> str:
    to_encode = data.copy()
    if expires_delta:
...
def get_current_user_from_token(token: str, secret_key: str, algorithm: str) -> UserInfo:
    try:
...
class UserService:
        ...
        async with session_manager.get_session() as session:
            existed_user: User = await user_repo.get_one(
                ...
                select(UserRepo.table)
                    .where((UserRepo.c.username == username) | (UserRepo.c.email == email))
                    .limit(1)
            )
    ...
    def get_user_info_from_token(self, token: str) -> UserInfo:
        return get_current_user_from_token(token, self.secret_key, self.algorithm)