...
class PostgresService:
    ...
    async def get_by_condition(self, repo: Type[PostgresRepo[T]], conditions: SqlConditionInterface) -> List[T]:
    ...
    @overload
    async def insert(self, repo: Type[PostgresRepo[T]], record: T | BaseDict, returning: Literal[True]) -> T: ...

    @overload
    async def insert(self, repo: Type[PostgresRepo[T]], record: T | BaseDict, returning: Literal[False]) -> None: ...

    @overload
    async def insert(self, repo: Type[PostgresRepo[T]], record: T | BaseDict) -> None: ...

    async def insert(self, repo: Type[PostgresRepo[T]], record: T | BaseDict, returning: bool = False) -> T | None:
    ...
    async def insert_on_conflict_do_nothing(
        self, repo: Type[PostgresRepo[T]], record: T | BaseDict, identity_columns: List[str], returning: Literal[True]
    ) -> T: ...

    @overload
    async def insert_on_conflict_do_nothing(
        self, repo: Type[PostgresRepo[T]], record: T | BaseDict, identity_columns: List[str], returning: Literal[False]
    ) -> None: ...

    @overload
    async def insert_on_conflict_do_nothing(
        self, repo: Type[PostgresRepo[T]], record: T | BaseDict, identity_columns: List[str]
    ) -> None: ...

    async def insert_on_conflict_do_nothing(
        self, repo: Type[PostgresRepo[T]], record: T | BaseDict, identity_columns: List[str], returning: bool = False
    ) -> T | None:
        query = repo.insert_on_conflict_do_nothing(
            record=record, identity_columns=identity_columns, returning=returning
        )
        cursor = await self.execute_raw_query(sql=query.sql, params=query.params)
        if returning:
            return repo.row_factory(cursor)[0]
    ...
    async def insert_many(
        self, repo: Type[PostgresRepo[T]], records: List[T | BaseDict], returning: Literal[True]
    ) -> List[T]: ...

    @overload
    async def insert_many(
        self, repo: Type[PostgresRepo[T]], records: List[T | BaseDict], returning: Literal[False]
    ) -> None: ...

    @overload
    async def insert_many(
        self, repo: Type[PostgresRepo[T]], records: List[T | BaseDict], returning: bool = False
    ) -> List[T] | None: ...
    ...
    async def lock_table(self, repo: Type[PostgresRepo[T]], mode="ACCESS EXCLUSIVE"):
        with self.session_scope():
            query = repo.lock_table(mode)
            await self.execute_raw_query(sql=query.sql, params=query.params)
        return

    async def get_order_columns(self, repo: Type[PostgresRepo[T]], exclude_columns: List[str] = []) -> List[str]:
