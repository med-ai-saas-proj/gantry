...
class SqlserverQueryBuilder:
        ...
        for sub_condition in conditions["conditions"]:
            if "conditions" in sub_condition:
                ...
                if isinstance(value, (list, tuple)):
                    value_sql = ", ".join([PLACEHOLDER for _ in value])
    ...
    @classmethod
    def fetch_next_offset(cls, fetch_next: int | None = None, offset: int | None = None):
        sql = []
        params = []
        if offset is not None:
            sql.append(f"OFFSET {PLACEHOLDER} ROWS")
            params.append(offset)
        if offset is not None:
            sql.append(f"FETCH NEXT {PLACEHOLDER} ROWS ONLY")
            params.append(fetch_next)
