...
class DocumentReader:
    ...
    async def extract_content(cls, content: bytes) -> str:
        ...
        if not content.startswith(b'PK'):
            raise ValueError("File is not a valid Word document (.docx). Only .docx format is supported.")
        ...
        try:
            ...
            if "no relationship" in error_msg or "officeDocument" in error_msg:
                raise ValueError("File is not a valid Word document (.docx). Please ensure it's a proper .docx file.")
            elif "corrupted" in error_msg or "bad zipfile" in error_msg:
                raise ValueError("Word document is corrupted or damaged. Please try with a different file.")
            else:
